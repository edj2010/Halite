...
class Command:
    ...
    def moveShipSmart(self, ship, target, unsafe = False):
        ...
        for dir in Direction.get_all_cardinals():
            ...
            heapq.heappush(toSearch, (cost(ship.position) + heuristic(newPos) - heuristic(ship.position), newPos, dir))
            ...
            if pos == target:
                ...
                for d in Direction.get_all_cardinals():
                    ...
                    if newPos not in seen:
                        heapq.heappush(toSearch, ((c + cost(pos) + heuristic(newPos) - heuristic(pos), newPos, dir)))
...
class Fleet:
            ...
            if self.inRegion(ship.position, pos):
                ...
                if self.goodTarget(pos) and ship.halite_amount < RETURN_T:
                    ...
                    if target:
                        ...
                        self.fleetOrders[ship.id] = ('mine', target, ('rand', None, ('hold', None)))
                    else:
                        # Return home
                        self.fleetOrders[ship.id] = ('move', self.command.shipyard.position, ('hold', None, ('rand', None, None)))
                    self._executeOrder(ship, self.fleetOrders[ship.id])
    ...
    # Returns said target
    def findNearTarget(self, ship, maxDis=5):
        toSearch = deque()
        seen = set()
        ...
        while toSearch:
            ...
            if not self.posAssigned(pos):
                bestSquare = max((self.command.getHalitePos(pos) / (depth + maxDis), pos), bestSquare)
            if depth < maxDis:
                for dir in Direction.get_all_cardinals():
