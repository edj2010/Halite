...
return_fac = 0.967#float(sys.argv[1])
mine_fac = 5.489#float(sys.argv[2])
build_fac = 0.388#float(sys.argv[3])

# Initialize and start game
game = hlt.Game()

## Constants
RETURN_T = int(constants.MAX_HALITE * return_fac) # 1 to 1000
MINE_T = int(constants.MOVE_COST_RATIO * mine_fac) # 1 to 1000
BUILD_TURN = int(constants.MAX_TURNS * build_fac) # 1 to max turns
...
class Command:
    ...
    def moveShipSmart(self, ship, target, unsafe = False):
        ...
        for dir in Direction.get_all_cardinals():
            ...
            heapq.heappush(toSearch, (cost(newPos) + heuristic(newPos) - heuristic(ship.position), newPos, dir))
            ...
            if pos == target:
                ...
                for d in Direction.get_all_cardinals():
                    ...
                    if newPos not in seen:
                        heapq.heappush(toSearch, ((c + cost(newPos) + heuristic(newPos) - heuristic(pos), newPos, dir)))
...
class Fleet:
    ...
    def evalTarget(self, target):
        h = self.command.getHalitePos(target)
    ...
    def buildShips(self):
        if self.command.me.halite_amount >= constants.SHIP_COST and self.command.game.turn_number < BUILD_TURN:
