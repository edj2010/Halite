...
# hyperparameters
return_rat = 0.905#float(sys.argv[1])
mine_rat = 4.5#float(sys.argv[2])
build_rat = 0.52#float(sys.argv[3])
growth_rat = 0.0029#float(sys.argv[4])

# Initialize and start game
game = hlt.Game()

## Constants
RETURN_T = int(constants.MAX_HALITE * return_rat)
MINE_T = int(constants.MOVE_COST_RATIO * mine_rat)
BUILD_TURN = int(constants.MAX_TURNS * build_rat)
GROWTH_RATE = growth_rat + 1
...
class Command:
    ...
    def moveShipSmart(self, ship, target, unsafe = False):
        ...
        def cost(halite, distance):
            adjustedInv = (ship.halite_amount - halite)/fastPow(GROWTH_RATE, distance)
